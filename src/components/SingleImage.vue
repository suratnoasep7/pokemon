              ...
              <nav>
                <ul class="pagination justify-content-center">
                  ...
                  <li class="page-item" v-for="pageitem in page">
                    <a v-on:click="getPage($attrs.type, $attrs.subtype, $attrs.supertype, pageitem)" class="page-link">{{ pageitem }}</a>
                  </li>
...
export default {
  ...
  methods: {
    ...
    getPage(type, subtype, supertype, page) {
      this.loading = true
      AXIOS.get('cards?types='+type+'&subtype='+subtype+'&supertype='+supertype+'&page='+page).then(
        response => {
          if (response.data.cards.length > 0) {
            this.loading = false
            this.response = response.data.cards
